...
command3[1] = "log"
command3[2] = "-r"
command3[3] = "0::" + mercurialRevision
command3[4] = "--follow"
command3[5] = "--prune"
...
def updateVersions = false

allSummaries.each {
   def string = it
   if (string.size() > 0 && string.contains(" 'Desc': ")) {
      // skip eis_acc and Reviewboard commits
      if (!(string.contains("eis_acc") || string.contains("ReviewBoard"))) {
	  def summary = string.split(" 'Desc': ")[1]

	  //skip merge, create/close branch commits 
	  if (!summary.toLowerCase().contains("merge") && !summary.toLowerCase().contains("merging") && !(summary.toLowerCase().contains("clos") && summary.toLowerCase().contains("branch")) && !(summary.toLowerCase().contains("creat") && summary.toLowerCase().contains("branch"))) {
        ...
        // subsystem dependency update versions commits by eis_acc exception
        else if (string.contains("eis_acc")) {
            def summary = string.split(" 'Desc': ")[1]
            if (summary.toLowerCase().contains("update versions")) {
                  updateVersions = true
            }
        }
   }
...
if (summaries.isEmpty()) {
  if (updateVersions) {
     println INFO + "Found update dependency versions commits, will continue without changelog update"
     return
   } 
   else {
...
// Append new changes to changelog file
...
outStream << "# Subsystem " + releaseVersion + " release changes\n\n"
